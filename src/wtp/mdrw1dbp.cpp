...
*		2) Ratios of raw water to Pre-RM Pre-Cl2 DBP formation from Gabi Solarik, 1998
*		3) Water quality input parameters from the RM influent
*		4) Time since influent of RM
*
* Notes:
*  1. Called from choose_model() whenever modrw1dbpflag == TRUE or when current unit
*     is an RM and there is modrw2dbpflag == TRUE.
...
{
   //  FILE *fptr2;
   /* Inputs to model (from influent of last Rapid Mix): */
   ...
   double doc_out;    /* Process effluent DOC (mg/L) */
   double br;         /* Bromide              (mg/L) */
   double br_ugl;     /* Bromide              (ug/L) */
   double cl2dose;    /* Free chlorine (mg/L) as Cl2 */
   double eff_hours;  /* Contact time to effluent of current unit process
			  since last cl2 add. pt. (hours) */
   double inf_hours;  /* Contact time to influent of current unit process
			  since last cl2 add. pt. (hours) */
   double inf_FreeCl; /* Unit Process influent free chlorine (mole/L) */
   double inf_nh2cl;  /* Unit Process influent free chlorine (mole/L) */

   /* Intermediates: */
   double doc_reduction; /* Fraction of DOC removed from RM influent to process effluent */
   ...
   /* Outputs: */
   /* The "delta"(s) are the DBPs predicted to be formed in the current unit process
   only based on formation during the contact time of the process as referenced
   from the last point of chlorination using WQ info. at RM influent */
   ...
   if (unit->eff.last_rm_inf != NULL)
   {
      ...
      /* Get most water quality parameters and Cl2 dose inputs from effluent of
     current unit process */
      ...
      br_ugl = br * 1000;
      cl2dose = unit->eff.cl2dose;
      // FreeCl    = unit->eff.FreeCl2;
      // nh2cl     = unit->eff.NH2Cl;

      /* Get TOC and UV to be used in DBP equations from RMinfluent since
     pre-chlorination factors takes into account simultaneous pre-cursor
     removal and chlorination */
      eff = &unit->eff.last_rm_inf->eff;
      doc = eff->TOC;
      uv = eff->UV;

      /* Contact time for this model will be that since the last Cl2 add. pt.
     which is contained in unit->eff.hours; */
      eff_hours = unit->eff.hours;
    ...
    */
      ...
      /* For pre-Cl2 Model, apply Solarik factors to Sohn raw water model predictions */
